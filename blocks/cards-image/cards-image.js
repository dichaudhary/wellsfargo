...
/**
 * Decorate the cards-image block.
 * 3-up grid of image-led cards. Each card has a featured image at top followed
 ...
 *
 * @param {Element} block The cards-image block element
 */
export default function decorate(block) {
  ...
  [...block.children].forEach((row) => {
    ...
    [...li.children].forEach((div) => {
      if (div.children.length === 1 && div.querySelector('picture')) {
        div.className = 'cards-image-card-image';
      } else {
        div.className = 'cards-image-card-body';
      }
    });
  ...
  ul.querySelectorAll('.cards-image-card-body a[href^="#tcm:"]').forEach((a) => {
  ...
  // punctuation).
  ul.querySelectorAll('.cards-image-card-body').forEach((body) => {
