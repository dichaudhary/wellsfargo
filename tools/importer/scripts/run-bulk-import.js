...
async function createBrowserContext(browser) {
  return browser.newContext({
    ...
    // Bypass site CSP so we can inject the import bundle on pages with strict
    // script-src policies (e.g. wellsfargo.com which only allows nonced and
    // self-origin scripts). Without this, the inline-script injection in
    // injectScript() is blocked.
    bypassCSP: true,
    extraHTTPHeaders: {
...
async function addStealthScripts(page) {
  await page.addInitScript(() => {
    ...
    const makePlugin = (name, description, filename, mimeType, mimeSuffixes, mimeDescription) => {
      const mime = {
        type: mimeType, suffixes: mimeSuffixes, description: mimeDescription, enabledPlugin: null,
      };
      const plugin = {
        name, description, filename, length: 1, 0: mime,
      };
...
async function main() {
  const {
    importScript, urlsFile, outputDir, disableHttp2,
  } = parseArgs();
